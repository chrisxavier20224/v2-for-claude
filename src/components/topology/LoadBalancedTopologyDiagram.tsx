/**
 * LoadBalancedTopologyDiagram — parameterized load-balanced SD-WAN topology
 *
 * Renders a dark "engineering drawing" style SVG showing the load-balanced
 * data path:
 *   Customer FW → SD-WAN Router → N SIMs → N Antennas → RF link → N Cell Towers
 *   → N independent direct-to-internet paths (one per carrier) → INTERNET
 *
 * Key differences vs BondedTopologyDiagram:
 *   - No cloud bonding server
 *   - No combined encrypted tunnel set
 *   - No fixed public IP box (IP is an optional upgrade — noted separately)
 *   - Per-session routing note shown next to the router
 *
 * Parameterized by `carriers` (2 | 3 | 4) so Pro, Ultrafast, and Enterprise
 * can share a single source of truth.
 *
 * NOTE: This is a best-guess rendering pending final confirmation from the
 * CTO (Andrew Somerville). When he confirms the exact load-balanced flow,
 * this component is the single place to update.
 */
...
interface LoadBalancedTopologyDiagramProps {
  carriers: CarrierCount;
  productName: string;
  throughputLabel: string;       // e.g. "Up to 400 Mbps combined"
  throughputSubLabel?: string;
  className?: string;
}

// Palette — matches BondedTopologyDiagram (dark engineering-drawing).
// Canvas bg matches the enclosing section (hsl(222,47%,11%) ≈ #0f172a)
// so the diagram blends seamlessly into the Network Architecture section.
const C = {
  bg: '#0f172a',
  border: '#cccccc',
  text: '#eeeeee',
...
// ── Tower cluster layouts (same as bonded) ─────────────────────
...
// ── Per-carrier direct-to-internet path vertical offsets ──────
// These are the Y positions where each carrier's independent path
// crosses the "session path" zone between towers and the internet.
const PATH_OFFSETS: Record<CarrierCount, number[]> = {
  2: [-22, 22],
  3: [-34, 0, 34],
  4: [-48, -16, 16, 48],
};
...
export const LoadBalancedTopologyDiagram: React.FC<LoadBalancedTopologyDiagramProps> = ({
  carriers,
  productName,
  throughputLabel,
  throughputSubLabel = 'combined across carriers',
  className = '',
}) => {
  const midY = 280;
  const simStartX = 400;
  ...
  const towerAnchorY = midY;

  // The bonded diagram has a cloud DC at cloudX = 785+dx with width 190.
  // For LB, we replace the whole cloud DC with a simpler INTERNET target
  // placed at the right edge, and show N independent session paths from
  // the tower cluster to it.
  const pathStartX = 720 + dx;
  const internetBoxX = 830 + dx;
  const internetBoxW = 130;
  const internetBoxY = midY - 30;
  const internetBoxH = 60;
  ...
  const towers = TOWER_LAYOUTS[carriers].map((t, i) => ({
    x: towerAnchorX + t.dx - 20,
    y: towerAnchorY + t.dy,
    color: CARRIER_COLORS[i],
  }));

  const paths = PATH_OFFSETS[carriers].map((off, i) => ({
    y: midY + off,
    color: CARRIER_COLORS[i],
  }));
  ...
  return (
    <div className={className} style={{ background: C.bg, padding: '16px' }}>
      ...
      >
        ...
        {/* Rendered only on the React component — downloadable PDF stays
            unbranded for Elevate partners to share with clients. */}
        <image
        ...
        <Node
          ...
          lines={[
            ...
            { t: 'Load balanced mode', s: 9, c: C.textSm },
          ]}
        />

        {/* Per-session routing label below the router — tightened so three
            lines clear the SIM card cluster below (simY = midY + 94). */}
        <text x={340} y={midY + 54} textAnchor="middle" fontSize={10} fontFamily={FONT} fill={C.text} fontWeight="bold">
          {throughputLabel}
        </text>
        <text x={340} y={midY + 68} textAnchor="middle" fontSize={9} fontFamily={FONT} fill={C.textSm}>
          {throughputSubLabel}
        </text>
        <text x={340} y={midY + 82} textAnchor="middle" fontSize={9} fontFamily={FONT} fill={C.green}>
          per-session routing
        </text>
        ...
        <text x={towerAnchorX + 3} y={midY + 92} textAnchor="middle" fontSize={9} fontFamily={FONT} fill={C.textSm}>Independent</text>
        <text x={towerAnchorX + 3} y={midY + 106} textAnchor="middle" fontSize={9} fontFamily={FONT} fill={C.textSm}>Carriers</text>

        {/* ── N SESSION PATHS (direct to internet) ─────────── */}
        <text x={(pathStartX + internetBoxX) / 2} y={midY - 55} textAnchor="middle" fontSize={9} fontFamily={FONT} fill={C.green}>Session</text>
        <text x={(pathStartX + internetBoxX) / 2} y={midY - 43} textAnchor="middle" fontSize={9} fontFamily={FONT} fill={C.green}>paths</text>
        ...
        {paths.map((p, i) => (
          <g key={`path-${i}`}>
            ...
            />
            <polygon
              points={`${internetBoxX},${p.y} ${internetBoxX - 8},${p.y - 4} ${internetBoxX - 8},${p.y + 4}`}
              fill={p.color}
            />
          </g>
        ))}
        ...
        <Node
          ...
          stroke={C.border}
          lines={[
            { t: 'INTERNET', b: true, s: 13 },
            { t: 'via each carrier', s: 9, c: C.textSm },
          ]}
        />

        {/* Fixed IP note (optional upgrade) */}
        <text x={internetBoxX + internetBoxW / 2} y={internetBoxY + internetBoxH + 24} textAnchor="middle" fontSize={9} fontFamily={FONT} fill={C.green}>
          Static public IP:
        </text>
        <text x={internetBoxX + internetBoxW / 2} y={internetBoxY + internetBoxH + 38} textAnchor="middle" fontSize={9} fontFamily={FONT} fill={C.textSm}>
          optional upgrade
        </text>
        ...
        <text x={40} y={H - 152} fontSize={12} fontFamily={FONT} fill={C.textSm}>• Per-session carrier selection</text>
        <text x={40} y={H - 132} fontSize={12} fontFamily={FONT} fill={C.textSm}>• Each carrier active simultaneously</text>
        <text x={380} y={H - 152} fontSize={12} fontFamily={FONT} fill={C.textSm}>• No cloud aggregation</text>
        <text x={380} y={H - 132} fontSize={12} fontFamily={FONT} fill={C.textSm}>• Lower latency path</text>
        ...
        <line x1={340} y1={H - 55} x2={380} y2={H - 55} stroke={C.green} strokeWidth={1.5} />
        <text x={388} y={H - 50} fontSize={11} fontFamily={FONT} fill={C.textSm}>Session path</text>
